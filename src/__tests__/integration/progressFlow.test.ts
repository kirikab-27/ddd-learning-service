...
describe('Progress Flow Integration', () => {
  ...
  it('should update completion rate in navigation', async () => {
    ...
    // Complete first lesson (1 of 21 = 5%)
    await completeUseCase.execute({
      courseId: 'ddd-practice',
      lessonId: 'lesson-1-1',
    });
    ...
    expect(afterOneLesson.completionRate).toBe(5);

    // Complete second lesson (2 of 21 = 10%)
    await completeUseCase.execute({
      courseId: 'ddd-practice',
      lessonId: 'lesson-1-2',
    });
    ...
    expect(afterTwoLessons.completionRate).toBe(10);
  });
